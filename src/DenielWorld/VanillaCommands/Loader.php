...
use DenielWorld\VanillaCommands\command\Ability;
use DenielWorld\VanillaCommands\command\AlwaysDay;
use DenielWorld\VanillaCommands\command\Clear;
use DenielWorld\VanillaCommands\command\Connect;
use DenielWorld\VanillaCommands\command\ImmutableWorld;
use DenielWorld\VanillaCommands\command\MobEvent;
use DenielWorld\VanillaCommands\command\PlaySound;
use DenielWorld\VanillaCommands\command\SetBlock;
use DenielWorld\VanillaCommands\command\SetMaxPlayers;
use DenielWorld\VanillaCommands\command\Tag;
...
class Loader extends PluginBase implements Listener {
	...
	public function onEnable() {
		...
		$this->getServer()->getCommandMap()->registerAll("vanillacommands", [new Ability("ability", $this), new AlwaysDay("alwaysday", $this), new Clear("clear", $this), new Connect("connect", $this), new SetBlock("setblock", $this), new SetMaxPlayers("setmaxplayers", $this), new PlaySound("playsound", $this), new MobEvent("mobevent", $this), new ImmutableWorld("immutableworld", $this), new Tag("tag", $this)]);
	}
	...
	public function mobSpawnEvent(EntitySpawnEvent $event) {
		if(in_array("events_enabled", $this->mobevents)) {
			$event->setCancelled();
		}
	}
	...
	public function pillagerEvent(EntitySpawnEvent $event) {
		if($event->getEntity()->getId() === EntityIds::VINDICATOR or $event->getEntity()->getId() === EntityIds::EVOCATION_ILLAGER) {
			if(in_array("minecraft:pillager_patrols_event", $this->mobevents)) {
				$event->setCancelled();
			}
		}
	}
	...
	public function traderEvent(EntitySpawnEvent $event) {
		if($event->getEntity()->getNameTag() === "Wandering Trader") {//Wandering trader id ain't even implemented in PMMP, gotta use a name dependency until/for future PMMP updates.
			if(in_array("minecraft:wandering_trader_event", $this->mobevents)) {
				$event->setCancelled();
			}
		}
	}
